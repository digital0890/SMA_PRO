...
# --------------------------------------------------
# Dark Modern Themed Streamlit App with Centered Layout
# --------------------------------------------------
...
# Title
st.markdown("<h1 class='app-title'>📈 Crypto & Gold — Supply & Demand Analysis</h1>", unsafe_allow_html=True)
st.markdown("<hr style='opacity:0.06; width:80%; margin: 0 auto;' />", unsafe_allow_html=True)
...
if symbol == "Gold":
    ...
    data = df.rename(columns={"Open": "Open","High": "High","Low": "Low","Close": "Close","Volume": "Volume"}).copy()
    if timeframe == "4h":
...
# Info row
st.markdown("<div class='streamlit-card' style='text-align:center; margin: 0 auto;'>"
            f"<span style='margin-right:20px;'>🔴 Supply Points: <b>{len(supply_idx_filtered)}</b></span>"
            f"<span>🟢 Demand Points: <b>{len(demand_idx_filtered)}</b></span>"
...
fig.add_trace(go.Scatter(x=data.index[supply_idx_filtered],y=data['High'].iloc[supply_idx_filtered]+offset,mode='markers',marker=dict(symbol='triangle-down', color='red', size=12),name='Supply'), row=1, col=1)
fig.add_trace(go.Scatter(x=data.index[demand_idx_filtered],y=data['Low'].iloc[demand_idx_filtered]-offset,mode='markers',marker=dict(symbol='triangle-up', color='green', size=12),name='Demand'), row=1, col=1)
fig.add_trace(go.Bar(x=up.index,y=up['Volume'],name="Up Volume",marker_color="green"), row=2, col=1)
fig.add_trace(go.Bar(x=down.index,y=down['Volume'],name="Down Volume",marker_color="red"), row=2, col=1)
fig.add_trace(go.Scatter(x=data.index,y=data['Volume_MA20'],mode="lines",name="MA20 Volume",line=dict(color="orange", width=2)), row=2, col=1)
fig.update_layout(template="plotly_dark",xaxis_rangeslider_visible=False,height=800,showlegend=True,hovermode='x unified')
